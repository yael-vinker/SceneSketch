...
# CUDA_VISIBLE_DEVICES=0 python scripts/resnet_vs_vit/vit_combined_fc.py --im_name "semi-complex.jpeg" --mask 1 --num_strokes 32
...
# clip_conv_layer_weights_int[-1] = 0.5
...
# for i in layers:
    ...
    # clip_conv_layer_weights_str = [str(j) for j in clip_conv_layer_weights_int]
    # clip_conv_layer_weights = ','.join(clip_conv_layer_weights_str)
test_name = f"{model[:3]}_l1_l2_fc1L2_{num_strokes}s_mask{mask_object}_{os.path.splitext(os.path.basename(file_))[0]}"
sp.run(["python", 
