...
def run(seed, wandb_name, output_dir, losses_all_n):
    ...
    # print("config", config)
    try:
        # print(os.path.isfile(f"{output_dir}/{wandb_name}/config.npy"))
        config = np.load(f"{output_dir}/{wandb_name}/config.npy",
                     allow_pickle=True)[()]
        # print(config["best_normalised_loss"])
    except Exception as e: print(e)
    # print("bla")
        # print("here")
    losses_all_n[wandb_name] = config["best_normalised_loss"]
...
if __name__ == "__main__":
    ...
    exit_codes = []
    manager = mp.Manager()
    losses_all_n = manager.dict()
    # losses_all_n = manager.dict()
    # iters_all = manager.dict()
    ...
    for j in range(args.num_sketches):
        ...
        if multiprocess:
            ...
            P.apply_async(run, (seed, wandb_name, output_dir, losses_all_n))
        else:
            run(seed, wandb_name, output_dir, losses_all_n)

    if multiprocess:
        P.close()
        P.join()  # start processes

    print("losses_all", losses_all_n)

    # sorted_final = dict(sorted(losses_all.items(), key=lambda item: item[1]))
    sorted_final_n = dict(sorted(losses_all_n.items(), key=lambda item: item[1]))
    ...
    if args.width_optim:
        ...
        shutil.rmtree(f"{output_dir}/{winning_trial}/mlps")
